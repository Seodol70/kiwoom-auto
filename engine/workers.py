            ...
            for code, row in top_df.iterrows():
                ...
                if snap is not None and getattr(self._cfg, "yosep_trend_enabled", True):
                    ...
                    if len(_cl) >= _need and len(_hi) >= _need and len(_lo) >= _need:
                        ...
                        # 방향 판단 — EMA20·ATR14 이용 (check_jdm_entry JDM_TREND_DOWN 동일 기준) [Phase 2] IndicatorService 호출로 변경
                    ...
                    if snap is None:
                        ...
                        if _is_eod_window:
                            ...
                            if not sig_type and len(snap.closes_1min) >= (self._cfg.jdm_ma_short + 1):
                                from scanner.indicator_service import IndicatorService as _is_gate  # [Phase 2]
                                    ...
                                    if breakout_reason:
                                        ...
                                        if not _breakout_trend_blocked:
                                            ...
                                            if pending is None:
                                                ...
                                                # [NEW] Fast-Track 로직
                                                _is_opening_slot = (_slot == "OPENING")
                        ...
                        if now_active and rising_edge and cooldown_ok and _per_min_ok:
                            ...
                            from scanner.indicator_service import IndicatorService as _is_ctx  # [Phase 2]
