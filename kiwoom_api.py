...
TR_DELAY_SEC = float(config.SCHEDULER.get("tr_delay_sec", 0.5))
...
class KiwoomManager(KiwoomProtocol):
    ...
    def __init__(self) -> None:
        ...
        # TR별 차단 시간 관리 (Circuit Breaker)
        self._tr_bans: dict[str, float] = {}
    ...
    def is_tr_banned(self, tr_code: str) -> bool:
        """해당 TR이 현재 서킷브레이커에 의해 차단된 상태인지 확인한다."""
        now = time.monotonic()
    ...
    def get_master_last_price(self, code: str) -> int:
        """종목의 기준가를 반환한다. (기준가 = 전일종가)"""
        return safe_int(self._ocx.dynamicCall("GetMasterLastPrice(QString)", [code]))
    ...
    def get_stock_info(self, code: str) -> Optional[dict]:
        ...
        for retry in range(2):
            ...
            d = self._tr_data
            logger.info("[TR진단] %s(opt10001) 응답 수신: %s", code, d)
            
            if d.get("현재가"):  # 유효한 응답
                current_price = safe_int(d.get("현재가"))
                base_price    = safe_int(d.get("기준가"))
                if base_price == 0:
                    base_price = safe_int(d.get("전일종가"))
                
                # [FIX] 서버 등락률 파싱 (필드명 혼용 대응)
                raw_pct = d.get("등락률") or d.get("등락율") or "0"
                change_pct = safe_float(raw_pct)

                # [FINAL] 기준가가 0이면 역산 또는 현재가로 강제 복구
                if base_price == 0 and current_price > 0 and change_pct != 0:
                    base_price = int(current_price / (1.0 + change_pct / 100.0))
                elif base_price == 0 and current_price > 0:
                    base_price = current_price
                
                # 등락률이 0이면 역산 시도
                if change_pct == 0.0 and base_price > 0:
                ...
                return {
                    "code":          code,
                    "name":          self._fix_enc(str(d.get("종목명", ""))).strip(),
                    "current_price": current_price,
                    "prev_close":    base_price,
                    "change_pct":    change_pct,
                    "change_amt":    safe_int(d.get("전일대비")),
    ...
    def get_master_price(self, code: str) -> int:
        """또 다른 기준가 조회 OCX 함수 (GetMasterPrice)"""
        # KOA_Functions를 통해 호출하는 경우가 많음
        raw: str = self._ocx.dynamicCall(
            "KOA_Functions(QString, QString)", ["GetMasterPrice", code]
        )
        return safe_int(raw)
    ...
    def get_index_info(self, index_code: str) -> Optional[dict]:
        ...
        ok = self._comm_rq(TR_INDEX_INFO, "index_info", "9300", timeout_ms=3_000)
        if not ok:
        ...
        if raw_current:
            ...
            # 등락률 필드 활용 (필드명 혼용 대응: 키움은 TR마다 등락률/등락율/전일대비율 등을 섞어 씀)
            change_pct = safe_float(
                d.get("등락률") or d.get("등락율") or d.get("전일대비율") or 
                d.get("등락") or d.get("등락율(%)") or "0"
            )
            
            # [진단] 만약 등락률이 0인데 전일대비 필드가 있다면 역산 시도
            if change_pct == 0.0:
                diff = safe_float(d.get("전일대비") or "0")
                if diff != 0 and base > 0:
                    change_pct = round(diff / base * 100, 2)
            
            # [FIX] 기준가가 0이면 역산 시도
            if base == 0 and current > 0 and change_pct != 0:
                base = round(current / (1.0 + change_pct / 100.0), 2)
    ...
    def _comm_rq(
        ...
        # Circuit Breaker: 해당 TR 코드가 차단 중인지 확인
        now = time.monotonic()
        ...
        try:
            ...
            if ret != ReturnCode.OK:
                logger.error("CommRqData failed - tr=%s ret=%d", tr_code, ret)
                if ret == -200:
                    # 해당 TR 코드만 15분 차단
                    self._tr_bans[tr_code] = now + 900
                    logger.critical("!!! Kiwoom TR Limit (-200) detected for %s !!! Pausing this TR for 15 mins.", tr_code)
                return False
            self._tr_loop = QEventLoop()
    ...
    def reset_tr_bans(self) -> None:
        """차단된 모든 TR 코드의 차단을 해제한다 (서킷 브레이커 초기화)."""
        cnt = len(self._tr_bans)
        self._tr_bans.clear()
    ...
    def _parse_top_volume_rows(self, tr_code: str, rq_name: str) -> list[dict]:
        ...
        for i in range(cnt):
            ...
            if price_v == 0:
                # 추가 후보군 시도
                for _p_cand in ("현재가", "현재가(원)", "종가", "현재가(원)"):
                    price_v = abs(safe_int(g(_p_cand)))
                    if price_v != 0: break
            ...
            chg_pct = safe_float(raw_chg_pct)
            
            # 만약 등락률이 0인데 전일대비나 현재가가 있다면 역산 시도
            if chg_pct == 0:
                price_now = price_v
                raw_diff = g("전일대비")
                diff = safe_int(raw_diff)
                if price_now > 0 and diff != 0:
                    # 현재가 문자열에 '-'가 있으면 하락, 없으면(혹은 '+') 상승으로 간주
                    is_down = "-" in g("현재가")
                    actual_diff = -diff if is_down else diff
                    prev_close_recovered = price_now - actual_diff
                    if prev_close_recovered > 0:
                        chg_pct = round((price_now - prev_close_recovered) / prev_close_recovered * 100, 2)
                        # [FIX] 역산된 기준가를 보관하여 하단 rows.append에서 사용
                        price_v_recovered = prev_close_recovered
                        logger.debug("[opt10030] 등락률 역산 성공: %s %d(%+d) -> %.2f%% | 기준가=%d", 
                                     code, price_now, actual_diff, chg_pct, price_v_recovered)
                    else:
                        price_v_recovered = 0
                else:
                    price_v_recovered = 0
            ...
            if i == 0:
                # [EXTREME DEBUG] 모든 필드 이름과 값을 로그로 출력하여 누락 필드 확인
                # 키움 TR의 모든 사용 가능한 필드를 유추하기 위해 시도
                all_possible_fields = [
                    "종목명", "현재가", "전일대비", "등락률", "거래량", "거래대금", "시가", "고가", "저가",
            ...
            rows.append({
                ...
                "prev_close":    (price_v_recovered if price_v_recovered > 0 else _resolve_prev_close(
                                     ...
                                     price_v,
                                     chg_pct,
                                 )) or abs(self.get_master_last_price(code)),
                "change_pct":    chg_pct,
                "rank":          i + 1,
            })
