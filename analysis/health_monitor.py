...
WATCHDOG_TIMEOUT_SEC = 30     # 이 시간 동안 ACK 없으면 프리징 판정 (opt10030 TR + 일봉체인 처리 시간 여유, 2026-04-23: 20→30초)
...
SYNC_TIMEOUT_SEC     = 600    # 10분간 동기화 없으면 경고/재연결 (2026-05-05)
...
class HealthMonitor:
    ...
    def __init__(
        ...
        self._last_sync_at:  float             = time.monotonic()
    ...
    def record_portfolio_sync(self) -> None:
        """잔고 동기화 성공 시 호출하여 데이터 신선도를 증명한다."""
    ...
    def _check_data_freshness(self) -> None:
        """10분 이상 잔고 동기화가 없으면 문제 상황으로 판단."""
        elapsed = time.monotonic() - self._last_sync_at
        if elapsed > SYNC_TIMEOUT_SEC:
            logger.critical(
                "[HealthMonitor] 데이터 신선도 위기! %.0f초간 잔고 동기화 없음 → 재연결 시도",
                elapsed
            )
            self._event_log.write("STALE_DATA", {"elapsed": elapsed})
            # 재연결 트리거 (ErrorTracker와 동일한 안전 재연결 경로 사용)
            self._last_sync_at = time.monotonic() # 중복 트리거 방지
            self._err_tracker._safe_reconnect()
