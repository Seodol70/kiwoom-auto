...
class FeedbackEngine:
    ...
    def compute_adjustments(
        ...
        if hits := category_hits.get(LossCat.OPENING_NOISE):
            n = len(hits)
            # entry_open_surge_max_opening: 건수에 비례해 5~15% 하향
            ...
            adjs.append(ParamAdjustment(
                ...
                reason=f"OPENING_NOISE {n}건 — OPENING 슬롯 등락률 상한 하향",
        ...
        if hits := category_hits.get(LossCat.HIGH_ENTRY_CHG):
            ...
            old = cur("entry_open_surge_max_opening", 7.0)
            # 손실 거래 평균 진입 등락률의 85% 또는 현재값 10% 하향 중 작은 값
            candidate = round(min(old * 0.90, avg_chg * 0.85), 1)
            new = max(candidate, PARAM_BOUNDS["entry_open_surge_max_opening"][0])
            if new < old:   # 이미 낮으면 스킵
                adjs.append(ParamAdjustment(
                    param="entry_open_surge_max_opening", old_val=old, new_val=new,
                    reason=f"HIGH_ENTRY_CHG {len(hits)}건, 평균진입등락률 {avg_chg:.1f}%",
        ...
        if hits := category_hits.get(LossCat.EARLY_REVERSAL):
            ...
            adjs.append(ParamAdjustment(
                ...
                reason=f"EARLY_REVERSAL {len(hits)}건, 평균보유 {avg_hold:.1f}분 — 관찰시간 연장",
                category=LossCat.EARLY_REVERSAL, confidence=0.65,
            ))

        # ⑤ STOP_LOSS_HIT ≥ 4건 → min_chejan_strength 상향
        if hits := category_hits.get(LossCat.STOP_LOSS_HIT):
            if len(hits) >= 4:
                old = cur("min_chejan_strength", 120.0)
                factor = min(1.12, 1.04 + 0.02 * (len(hits) - 4))
                new = round(old * factor, 1)
                adjs.append(ParamAdjustment(
                    param="min_chejan_strength", old_val=old, new_val=new,
                    reason=f"STOP_LOSS_HIT {len(hits)}건 — 진입 체결강도 기준 강화",
    ...
    def run_daily(self, target_date: Optional[date] = None) -> FeedbackResult:
        ...
        흐름:
          ...
          4. 손실 당일만: 손실 원인 분류 + 파라미터 조정
          5. 시간대 조정 + 손실 조정 합산 → safety guards → write
        """
        ...
        # ── Step 2: 손실 원인 조정 (손실 당일 + 최소 건수 충족 시만) ────────
        loss_adjs:      List[ParamAdjustment] = []
        category_counts: Dict[str, int]       = {}

        if not profitable and len(filled) >= MIN_TRADES_FOR_ANALYSIS:
            category_hits_map = self.classify_losses(audits)
            category_counts   = {k: len(v) for k, v in category_hits_map.items()}
            logger.info("[Feedback] 카테고리 분류: %s", category_counts)
            loss_adjs = self.compute_adjustments(category_hits_map, existing_params)
        elif profitable:
        ...
        raw_adjs = time_adjs + loss_adjs
        approved, skipped = self.apply_safety_guards(raw_adjs, existing_params, history)

        # next_lock 이 계산됐으면 params 에 반영 (직접 float 저장)
        if next_lock > 0:
            # safety guard 없이 직접 덮어쓰기 (단순 통계 기반 업데이트)
            existing_params["daily_profit_lock_won"] = float(next_lock)
            logger.info("[Feedback] daily_profit_lock_won → %d원 (5일 평균 peak %.0f × %.0f%%)",
                        next_lock,
        ...
        logger.info(
            "[Feedback] 완료 — 손익 %.0f원 | %d건 | 슬롯조정 %d개 | 손실조정 %d개 | 승인 %d개 | 스킵 %d개",
            total_realized, len(filled), len(time_adjs), len(loss_adjs), len(approved), len(skipped),
        )
